...
class Gridmodel {
    ...
    */
    constructor(name, config, rng) {
        ...
        this.moore9 = [[ -4, -4 ], [ -3, -4 ], [ -2, -4 ], [ -1, -4 ], [ 0, -4 ],
        [ 1, -4 ],  [ 2, -4 ],  [ 3, -4 ],  [ 4, -4 ],  [ -4, -3 ],
        [ -3, -3 ], [ -2, -3 ], [ -1, -3 ], [ 0, -3 ],  [ 1, -3 ],
        [ 2, -3 ],  [ 3, -3 ],  [ 4, -3 ],  [ -4, -2 ], [ -3, -2 ],
        [ -2, -2 ], [ -1, -2 ], [ 0, -2 ],  [ 1, -2 ],  [ 2, -2 ],
        ...
        this.moore11 = [[-5,-5],[-4,-5],[-3,-5],[-2,-5],[-1,-5],[0,-5],[1,-5],
    ...
    */
    ...
    /** Get a neighbour at compass direction in a 5X5 Moore neighbourhood
     * @param {Gridmodel} grid The gridmodel used to check neighbours. Usually the gridmodel itself (i.e., this), 
    *  but can be mixed to make grids interact.
    *  @param {int} col position (column) for the focal gridpoint
    *  @param {int} row position (row) for the focal gridpoint
    *  @param {int} direction the neighbour to return 
    */
    getNeighbour5(model, col, row, direction) {
        let i = model.moore5[direction][0];
        let j = model.moore5[direction][1];
        return model.getGridpoint(col + i, row + j)
    }

    /** Get a neighbour at compass direction in a 7X7 Moore neighbourhood
     * @param {Gridmodel} grid The gridmodel used to check neighbours. Usually the gridmodel itself (i.e., this), 
    *  but can be mixed to make grids interact.
    *  @param {int} col position (column) for the focal gridpoint
    *  @param {int} row position (row) for the focal gridpoint
    *  @param {int} direction the neighbour to return 
    */
    getNeighbour7(model, col, row, direction) {
            let i = model.moore7[direction][0];
            let j = model.moore7[direction][1];
            return model.getGridpoint(col + i, row + j)
    }

    /** Get a neighbour at compass direction in a 9X9 Moore neighbourhood
     * @param {Gridmodel} grid The gridmodel used to check neighbours. Usually the gridmodel itself (i.e., this), 
    *  but can be mixed to make grids interact.
    *  @param {int} col position (column) for the focal gridpoint
    *  @param {int} row position (row) for the focal gridpoint
    *  @param {int} direction the neighbour to return 
    */
    getNeighbour9(model, col, row, direction) {
        let i = model.moore9[direction][0];
        let j = model.moore9[direction][1];
        return model.getGridpoint(col + i, row + j)
    }

    /** Get a neighbour at compass direction in a 11X11 Moore neighbourhood
    * @param {Gridmodel} grid The gridmodel used to check neighbours. Usually the gridmodel itself (i.e., this), 
    *  but can be mixed to make grids interact.
    *  @param {int} col position (column) for the focal gridpoint
    *  @param {int} row position (row) for the focal gridpoint
    *  @param {int} direction the neighbour to return 
    */
    getNeighbour11(model, col, row, direction) {
        let i = model.moore11[direction][0];
        let j = model.moore11[direction][1];
        return model.getGridpoint(col + i, row + j)
    }
    ...
    *  this.getMoore8(this,10,10,3,'species')
    */
    /*getPublicGoodsNeighbours(model,col,row,property,range) {
        let gps = [];
        for (let n = range[0]; n <= range[1]; n++) {                        
            let i = model.moore[n][0];
            let j = model.moore[n][1];
    ...
    getPublicGoodsMoore9(model,col,row,property,range) { return this.getPublicGoodsNeighbours(model,col,row,property,[0,8])} */
